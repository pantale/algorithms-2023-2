...
%=================================================================
\documentclass[algorithms,article,accept,pdftex,oneauthor]{Definitions/mdpi}
...
\pubyear{2023}
\copyrightyear{2023}
\externaleditor{Academic Editors: Nuno Fachada and Nuno David}
\datereceived{27 October 2023}
\daterevised{21 November 2023} % Comment out if no revised date
\dateaccepted{24 November 2023}
\datepublished{25 November 2023}
%\datecorrected{} % For corrected papers: "Corrected: XXX" date in the original paper.
%\dateretracted{} % For corrected papers: "Retracted: XXX" date in the original paper.
\hreflink{https://doi.org/10.3390/a16120537} % If needed use \linebreak
%\doinum{}
...
\lstset{language=Python,linewidth=0.95\linewidth,
...
\Author{Olivier %MDPI: Please carefully check the accuracy of names and affiliations. OP: It's Ok
 Pantal\'{e} \orcidA{}}
...
\AuthorCitation{Pantal\'{e}, O. %MDPI: Please carefully check the accuracy of names and affiliations. OP: It's Ok
}
% If this is a Chicago style journal: Lastname, Firstname, Firstname Lastname, and Firstname Lastname.

% Affiliations / Addresses (Add [XX] after \address if there is only one affiliation.)
\address[1]{Laboratoire Génie de Production, Institut National Polytechnique/%MDPI: please check if it should be comma or just keep it. OP: Keep the slash as usual
Ecole Nationale d'Ingénieurs de Tarbes, Université de Toulouse, 47 Av d'Azereix, F-65016 Tarbes, France; olivier.pantale@enit.fr; %MDPI: we changed it into lowercase, please check. OP: It's Ok
 Tel.: +33-562-442-933}
...
% Keywords
\keyword{\textls[-15]{constitutive behavior; ANN flow law; numerical implementation; user hardening; activation functions; abaqus}}
...
The general form of the flow law is expressed through a mathematical equation: %MDPI: Please confirm if the bold in equation is unnecessary and can be removed. The following highlights are the same. please check all the equations
%OP: It's Ok to keep the bold letter to have a difference between stress and function
\begin{equation}
...
\caption{Two %MDPI: Please check if the color in figure need more explanations. OP: No need here.
...
This %MDPI: please check if the noindent of the paragraph after equations should be kept or revised. please check all the paragraph after equations. OP: It's Ok
...
Therefore, we apply the same procedure as previously presented and the flow stress~$\sigma$ is related to the output~$s$ according to the expression: %MDPI: we revise the dot into center dots, please check all the equations. OP: Ok
\begin{equation}
...
Therefore, as Equations (\ref{eq:ANN-y})--(\ref{eq:ANN-s}) are used to compute~$\overrightarrow{y}\lay{k}$ and~$\overrightarrow{\hat{y}}\lay{k}$ for each hidden layer and the output~$s$ from the input vector~$\overrightarrow{x}$ of the ANN, we can write the derivative~$\overrightarrow{s}'$ of a two hidden layers network as follows: %MDPI: please check if the cicle in equations are correct, please check all equations. OP: It's Ok
\begin{equation}
...
All six models underwent parallel training for a consistent number of iterations ($6000$ iterations, lasting around 1 h), on a Dell PowerEdge R730 %MDPI: Please state the name of the manufacturer, city, and country from where the equipment was sourced. OP : I have added the name of the constructor, it is sufficient for this type of computer.
...
\begin{table}[H]
\caption{Comparison %MDPI: 1. we removed the vertical line, please check. 2. we changed the alignments into center/middle, please check. 3. We add and revised the format in table header, please check OP : It's Ok for me
 of the models' error depending on the activation function used.\label{tab:Training}}
\newcolumntype{C}{>{\centering\arraybackslash}X}
\begin{tabularx}{\textwidth}{CCCCCCcccc}
\toprule
\textbf{Activation} & \textbf{CPU} & \textbf{$\MSE$} \boldmath{$\times 10^{-6}$} & \textbf{$\RMSE$} \textbf{(\MPa)}& \boldmath{$\Delta$}\textbf{$\RMSE$ (\%)}& \textbf{$\MARE$} & \boldmath{$\Delta$}\textbf{$\MARE$} &  \boldmath{$\Delta E$} &\textbf{ Rank} \\\midrule
...
From %MDPI: we moved the position of paragraph as the blank in pages, please check. OP : It's Ok for me
...
\textls[-15]{The Fortran subroutine is compiled with double precision directive using the Intel Fortran 14.0.2 compiler on a Ubuntu 22.04 server and linked to the main Abaqus explicit~executable.}
...
\caption{Half %MDPI: please check if the red arrow and blue circle need more explanations. OP : It's Ok for me it clear 
...
\toprule
\textbf{Activation} & \textbf{CPU (s)} & \boldmath{$N_{inc}$} & \boldmath{$N_{inc}$}\textbf{/s} & \boldmath{$\overline{\varepsilon}^p$} \textbf{(\MPa)}& \boldmath{$\overline{\sigma}$}  & \boldmath{$T$} \textbf{(\celsius)}\\ \midrule
...
\caption{Equivalent %MDPI: 1. Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1". 2. Please change the terms into scientific notations in the figure,  e.g., "$8 \times 10^{3}$", not "8E3". the same as Figure~10. OP: I'ts Ok now
...
\caption{\textls[-15]{Von Mises stress vs. displacement of the top edge of the cylinder for all activation functions.}}
...
\acknowledgments{The author thanks the team of Mohammad %MDPI: honor title is not allowed in this part, so we deleted ``Pr.'', please check OP: Ok for me
 Jahazi from the Ecole Technique Supérieure de Montréal, Canada, for providing the experimental data used in Section~\ref{subsec:ExpTests}.}
...
\end{PythonListing}}
\end{tabular}
\caption{Python %MDPI: when we run the tex zip, we find the Figures A1--A4 are little different form the pdf sumbmitted on susy, please check if they are correct in tex zip. OP: Figures A1--A4 are note included in the zip file but generated by the piece of code above during Latex Compilation. I checked that the text is the same. Ok for me !
 function to compute the flow stress and the derivative vector.\label{fig:PythonStress}}
\end{figure}
The network architecture is defined by the numpy arrays \var{w1}, %MDPI: please check if the special font shpuld be kept, please check whole Appendix. OP: Yes, keep this font to be in accordance with the one in the Figure.
...
\bibitem[Abaqus(1989)]{Abaqus}
Abaqus.
\newblock {\em Reference Manual}; Hibbitt, Karlsson and Sorensen Inc.: Providence, RI, USA, 1989. 


\bibitem[Lin et~al.(2009)Lin, Chen, and Zhang]{Lin-2009-MFS}
...
499},~88--92.
\newblock  [\href{http://doi.org/10.1016/j.msea.2007.11.119}{CrossRef}]

\bibitem[Bennett et~al.(2010)Bennett, Leen, Williams, Shipway, and
Hyde]{Bennett-2010-C}
...
\newblock {\em Comput. Mater. Sci.} {\bf 2010}, {\em 50},~125--137. [\href{http://dx.doi.org/10.1016/j.commatsci.2010.07.016}{CrossRef}]

\bibitem[Kumar(2016)]{Kumar-2016-TMS}
Kumar, V.
\newblock Thermo-mechanical simulation using gleeble system-advantages and
limitations.
\newblock {\em J. Metall. Mater. Sci.} {\bf 2016}, {\em
58},~81--88.

\bibitem[Yu et~al.(2019)Yu, Xu, Wang, Zhao, Wei, and Yang]{Yu-2019-RCR}
...
\newblock {\em J. Mater. Sci. Technol.} {\bf 2019}, {\em
35},~1039--1043.
\newblock   [\href{http://dx.doi.org/10.1016/j.jmst.2018.12.026}{CrossRef}]

\bibitem[Kolsky(1949)]{Kolsky-1949-IMP}
Kolsky, H.
\newblock An {Investigation} of the {Mechanical} {Properties} of {Materials} at
very {High} {Rates} of {Loading}.
\newblock {\em Proc. Phys. Soc. Sect. B} {\bf 1949}, {\em
62},~676--700.
\newblock   [\href{http://dx.doi.org/10.1088/0370-1301/62/11/302}{CrossRef}]

\bibitem[Ponthot(2002)]{Ponthot-2002-USU}
Ponthot, J.P.
...
\newblock {\em Int. J. Plast.} {\bf 2002}, \emph{18}, 
36. [\href{http://dx.doi.org/10.1016/S0749-6419(00)00097-8}{CrossRef}]

\bibitem[Johnson and Cook(1983)]{Johnson-1983-CMD}
...
\bibitem[Zerilli and Armstrong(1987)]{Zerilli-1987-DMB}
...
\bibitem[Jonas et~al.(1969)Jonas, Sellars, and Tegart]{Jonas-1969}
...
\bibitem[Ming and Pantal{\'e}(2018)]{Ming-2018-ERV}
Ming, L.; Pantal{\'e}, O.
...
\bibitem[Liang et~al.(2022)Liang, Kong, Zhang, and Li]{Liang-2022}
...
\newblock   [\href{http://dx.doi.org/10.1002/srin.202200443}{CrossRef}]

\bibitem[{Tize Mha} et~al.(2023){Tize Mha}, Dhondapure, Jahazi, Tongne, and
Pantal{\'e}]{Tize-2023-IEP}
...
\newblock {\em Metals} {\bf 2023}, {\em 13},~633. [\href{http://dx.doi.org/10.3390/met13030633}{CrossRef}]

\bibitem[Pantalé et~al.(2022)Pantalé, {Tize Mha}, and
Tongne]{Pantale-2021-EIN}
...
198},~103647.
\newblock   [\href{http://dx.doi.org/10.1016/j.finel.2021.103647}{CrossRef}]

\bibitem[Pantalé(2023)]{Pantale-2023-DIA}
Pantalé, O.
...
Software.
\newblock {\em Algorithms} {\bf 2023}, {\em 16}, 56.
\newblock   [\href{http://dx.doi.org/10.3390/a16010056}{CrossRef}]

\bibitem[Minsky and Papert(1969)]{Minsky-1969-PIC}
Minsky, M.L.; Papert, S.
\newblock {\em Perceptrons; An {{Introduction}} to {{Computational Geometry}}};
MIT Press: Cambridge, UK, 
1969.

\bibitem[Hornik et~al.(1989)Hornik, Stinchcombe, and White]{Hornik-1989-MFN}
Hornik, K.; Stinchcombe, M.; White, H.
\newblock Multilayer Feedforward Networks Are Universal Approximators.
\newblock {\em Neural Net.} {\bf 1989}, {\em 2},~359--366. [\href{http://dx.doi.org/10.1016/0893-6080(89)90020-8}{CrossRef}]

\bibitem[Gorji et~al.(2020)Gorji, Mozaffar, Heidenreich, Cao, and
Mohr]{Gorji-2020}
...
\bibitem[Lin et~al.(2008)Lin, Zhang, and Zhong]{Lin-2008}
...
\bibitem[Stoffel et~al.(2018)Stoffel, Bamer, and Markert]{Stoffel-2018-ANN}
...
\newblock {\em Thin-Walled Struct.} {\bf 2018}, {\em 131},~102--106. [\href{http://dx.doi.org/10.1016/j.tws.2018.06.035}{CrossRef}]

\bibitem[Stoffel et~al.(2019)Stoffel, Bamer, and Markert]{Stoffel-2019-NNB}
Stoffel, M.; Bamer, F.; Markert, B.
...
\bibitem[Cheng et~al.(2022)Cheng, Wang, Zhou, Zuo, and Zhang]{Cheng-2022-CWD}
...
\newblock {\em Metals} {\bf 2022}, {\em 12}, 1429.
\newblock   [\href{http://dx.doi.org/10.3390/met12091429}{CrossRef}]

\bibitem[Churyumov and Kazakova(2023)]{Churyumov-2023-PTS}
...
\newblock {\em Materials} {\bf 2023}, {\em 16}, 1083.
\newblock   [\href{http://dx.doi.org/10.3390/ma16031083}{CrossRef}] [\href{http://www.ncbi.nlm.nih.gov/pubmed/36770090}{PubMed}]

\bibitem[Dubey et~al.(2022)Dubey, Singh, and Chaudhuri]{Dubey-2022-AFD}
Dubey, S.R.; Singh, S.K.; Chaudhuri, B.B.
\newblock Activation functions in deep learning: A comprehensive survey and
benchmark.
\newblock {\em Neurocomputing} {\bf 2022}, {\em 503},~92--108.
\newblock   [\href{http://dx.doi.org/10.1016/j.neucom.2022.06.111}{CrossRef}]

\bibitem[Jagtap and Karniadakis(2023)]{Jagtap-2023-HIA}
...
Le]{Ramachandran-2018-SAF}
Ramachandran, P.; Zoph, B.; Le, Q.V.
\newblock Searching for Activation Functions. \emph{arXiv} \textbf{2018}, arXiv:1710.05941 

\bibitem[Shen et~al.(2021)Shen, Yang, and Zhang]{Shen-2021-NNA}
Shen, Z.; Yang, H.; Zhang, S.
\newblock Neural network approximation: Three hidden layers are enough.
\newblock {\em Neural Net.} {\bf 2021}, {\em 141},~160--173.
\newblock   [\href{http://dx.doi.org/10.1016/j.neunet.2021.04.011}{CrossRef}]

\bibitem[Abadi et~al.(2015)Abadi, Agarwal, Barham, Brevdo, Chen, Citro, and
Corrado]{Tensorflow-2015}
...
\bibitem[Kingma and Lei(2014)]{Kingma-2015-AMS}
Kingma, D.P.; Lei, J.
\newblock Adam:  A Method  for  Stochastic Optimization.
\newblock {\em arXiv} {\bf 2014},  
arXiv:1412.6980. 

\bibitem[Pantalé(2023)]{Pantale-2023-SSF}
Pantalé, O.
...
\bibitem[Koranne(2011)]{Koranne-2011-HDF}
Koranne, S.
