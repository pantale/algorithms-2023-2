...
With regard to our previous publications about ANN constitutive flow law~\cite{Pantale-2021-EIN}, we have made the choice to arbitrary fix some hyper-parameters of the ANNs, so to use a two hidden layers ANN with \textcolor{red}{15} neurons for the first hidden layer and \textcolor{red}{7} neurons for the second hidden layer.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\columnwidth]{Figures/3Cr2Mo-convergence}
...
As expected, the ReLU activation function gives the worst results with a final value of~$\MSE=31.537\times10^{-6}$, mainly due to the low number of neurons and the fact that this function is a piecewise linear function and not able to efficiently approximate the nonlinear behavior of the material.
...
Of the six activation functions used in this study, the Exponential function gives the best results in terms of solution quality, while the ReLU function gives the worst \textcolor{red}{as reported by $\Delta E=\sqrt{\RMSE^2+\MARE^2}$}.
