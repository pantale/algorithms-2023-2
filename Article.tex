...
Of course there is no limitation to the use of alternative activation functions in ANNs and there exist some much more complicated such as the one proposed by Shen \eal~\cite{Shen-2021-NNA} which is a combination of a floor, an exponential and a step function. Those authors have proved that a three hidden layer with this activation function can approximate any Hölder continuous function with an exponential approximation rate.
...
As proposed in Pantalé \eal~\cite{Pantale-2021-EIN}, and with regard to considerations concerning the effect of the strain rate over the evolution of the flow stress, we first substitute~$\ln(\mdot{\varepsilon}/\mdot{\varepsilon}_0)$, with~$\mdot{\varepsilon}_0$ equal to the lowest strain rate, for the value of~$\mdot{\varepsilon}$.
...
The output of the model, \ie the flow stress~$\sigma$, enhances the same kind of behavior with~$\Delta\sigma=153.7~\MPa$ and~$\overline\sigma=63.6~\MPa$.
...
\end{tabular}
\end{table}
We can see from this later that all models need more or less the same training time to complete the requested number of iterations. We can see that the complexity of the activation functions has an influence on the training time, since it is greater for the Swish and Softplus functions than for the ReLU one.
We also reported in this table the real values of the~$\RMSE$ and~$\MARE$ concerning the flow stress~$\sigma$. From this later we can see that the~$\RMSE$ is about~$0.3$ to~$0.4~\MPa$ for all activation functions except the ReLU one where the value is above~$1.1~\MPa$.
...
\end{figure}
We can see that over the whole range of temperatures~$T$ and strain rates~$\mdot{\varepsilon}$, the performance of the model based on an exponential function is very good overall. This model will therefore be retained for the remainder of the comparative study.
...
We suppose that the current input is stored in a three components vector $\overrightarrow{\xi}$, with $\xi_1=\varepsilon$, $\xi_2=\log(\mdot{\varepsilon}/\mdot{\varepsilon}_0)$ and~$\xi_3=T$.
We also suppose that the minimum and maximum values of the inputs, used during the learning phase, are stored in two vectors $\overrightarrow{\xi}_{min}$ and $\overrightarrow{\xi}_{max}$ respectively. The minimum and maximum values of the stress are also stored in two variables $\sigma_{min}$ and $\sigma_{max}$ respectively.
\begin{itemize}
...
\begin{equation}
\overrightarrow{x}=\left(\overrightarrow{\xi}-\overrightarrow{\xi}_{min}\right)\oslash\Delta\overrightarrow{\xi},
\end{equation}
where $\oslash$ is the Hadamard division operator and $\Delta\overrightarrow{\xi}=\overrightarrow{\xi}_{max}-\overrightarrow{\xi}_{min}$.
...
\begin{equation}
s = \Delta\sigma.\left(\overrightarrow{w}^T \dotp \overrightarrow{\hat{y}}\lay{2} + b\right) + \sigma_{min}
\end{equation} 
...
\begin{equation}
\overrightarrow{\sigma}' = \Delta\sigma.\w\lay{1}^T \dotp\left[\left(\w\lay{2}^T \dotp \left(\overrightarrow{w}^T \circ f'(\overrightarrow{y}\lay{2})\right)\right) \circ f'(\overrightarrow{y}\lay{1})\right] \oslash \left(\Delta\overrightarrow{\xi} \circ \overrightarrow{u}\right),
\end{equation}
where $\overrightarrow{u}^T=[1,\mdot{\varepsilon},1]$.
\end{itemize}

\begin{figure}[H]
\begin{PythonListing}%[float,floatplacement=H]
def AnnFunctionWithDerivatives(xi, act, dact):
    xi[1,:] = np.log(xi[1,:] / deps0)
    x = (xi - xi_min) / delta_xi
    y1 = w1.dot(x) + b1
    h1 = act(y1)
    y2 = w2.dot(h1) + b2
    h2 = act(y2)
    sigma = (w3.dot(h2) + b3) * delta_sig + sig_min
    dsigma = (w1.T).dot((w2.T).dot(w3.T * dact(y2)) 
    	* dact(y1)) * delta_sig / delta_xi
    dsigma[1,:] = dsigma[1,:] / xi[1,:]
    return sigma, dsigma 
\end{PythonListing}
\caption{L}
\end{figure}
